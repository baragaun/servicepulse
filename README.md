...
* `clone git@github.com:edwinhern/express-typescript-2024.git`
* Rename the folder to `servicepulse`
* `cd servicepulse`
* `npm install git+ssh://git@github.com:baragaun/e2etestrunner.git`
* `mkdir config`
* Copy `mmdata/tools/e2etestrunner/config/cicd` into `config`
* -----------
* Add an API endpoint just like `healthCheck` and call it `serviceStatus`
* In `serviceStatus`, return the status you get from `import { getStatus } from 'services/mmdata';` 
* Find a good scheduling npm package, such as https://www.npmjs.com/package/node-schedule
* Using the scheduling package, call `check` from `mmdata` every 10 minutes
